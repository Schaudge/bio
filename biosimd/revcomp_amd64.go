...
func ReverseComp8InplaceNoValidate(ascii8 []byte) {
	...
	reverseCompInplaceLookupSSSE3Asm(unsafe.Pointer(ascii8Header.Data), &revComp8Table16, nByte)
}
...
func ReverseComp4UnsafeInplace(seq8 []byte) {
	...
	if nByte <= 16 {
		reverseCompInplaceTinyLookupSSSE3Asm(unsafe.Pointer(seq8Header.Data), &revComp4Table, nByte)
		return
	}
	reverseCompInplaceLookupSSSE3Asm(unsafe.Pointer(seq8Header.Data), &revComp4Table, nByte)
}
...
func ReverseComp4Unsafe(dst, src []byte) {
	...
	dstHeader := (*reflect.SliceHeader)(unsafe.Pointer(&dst))
	if nByte <= 16 {
		reverseCompTinyLookupSSSE3Asm(unsafe.Pointer(dstHeader.Data), unsafe.Pointer(srcHeader.Data), &revComp4Table, nByte)
		return
	}
...
func ReverseComp2UnsafeInplace(acgt8 []byte) {
	...
	if nByte <= 16 {
		reverseCompInplaceTinyLookupSSSE3Asm(unsafe.Pointer(acgt8Header.Data), &revComp2Table, nByte)
		return
	}
	reverseCompInplaceLookupSSSE3Asm(unsafe.Pointer(acgt8Header.Data), &revComp2Table, nByte)
}
...
func ReverseComp2Inplace(acgt8 []byte) {
	// This has no performance penalty relative to the unsafe function when nByte
	// > 16, but there's a significant penalty below that.
	nByte := len(acgt8)
	if nByte <= 16 {
...
func ReverseComp2Unsafe(dst, src []byte) {
	...
	dstHeader := (*reflect.SliceHeader)(unsafe.Pointer(&dst))
	if nByte <= 16 {
		reverseCompTinyLookupSSSE3Asm(unsafe.Pointer(dstHeader.Data), unsafe.Pointer(srcHeader.Data), &revComp2Table, nByte)
		return
	}
